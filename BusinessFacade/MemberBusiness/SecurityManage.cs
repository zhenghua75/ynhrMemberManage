...
namespace ynhrMemberManage.BusinessFacade.MemberBusiness
{
	...
	public class SecurityManage
	{
        ...
        public List<string> GetFuncById(int operid, int deptid)
        {
            List<string> lFunc = new List<string>();
            //系统管理员具有所有权限				
            //否则比对权限列表
            DataTable dtOperFunc = SqlHelper.ExecuteDataTable(CommandType.Text, "select * from tbOperFunc where cnnOperID = " + operid);
            ...
            DataTable dtDeptFunc = SqlHelper.ExecuteDataTable(CommandType.Text, "select * from tbDeptFunc where cnnDeptID = " + deptid);
            foreach (DataRow dr in dtDeptFunc.Rows)
            {
                FuncList funcList = new FuncList(dr);
        ...
        public void AddDeptFunc(List<string> lfunc, string deptid)
        {
            ...
            try
            {
                conn = ConnectionPool.BorrowConnection();
                trans = conn.BeginTransaction();
                SqlHelper.ExecuteNonQuery(trans, CommandType.Text, "delete from tbDeptFunc where cnnDeptID = " + deptid);
		...
		public void ModifyOperFunc(List<string> alOperFuncList,string iOperID)
		{
			...
			try
			{
				conn = ConnectionPool.BorrowConnection();
				trans = conn.BeginTransaction();
				SqlHelper.ExecuteNonQuery(trans,CommandType.Text,"delete from tbOperFunc where cnnOperID = "+iOperID);
				for (int i = 0;i<alOperFuncList.Count;i++)
				{
