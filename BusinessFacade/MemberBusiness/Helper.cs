...
namespace ynhrMemberManage.BusinessFacade.MemberBusiness
{
	...
	public class Helper
	{
        ...
        public static string CreateCardNo6()
        {
            string strRet = "";
            string strCount = SqlHelper.ExecuteScalar(CommandType.Text, "select count(*) from tbMember where Len(cnvcMemberCardNo)=6 ").ToString();

            int iCount = int.Parse(strCount) + 1;
            strRet = GetCardNo(iCount);
            string strsql = "select  count(*) from tbMember where Len(cnvcMemberCardNo)=6 and cnvcMemberCardNo='" + strRet + "'";
            strCount = SqlHelper.ExecuteScalar(CommandType.Text, strsql).ToString();
            int icount2 = Convert.ToInt32(strCount);
            if (icount2 > 0)
            {
                strsql = "select  MAX(cnvcMemberCardNo) from tbMember where Len(cnvcMemberCardNo)=6";
                string strCardNo = SqlHelper.ExecuteScalar(CommandType.Text, strsql).ToString();
                iCount = Convert.ToInt32(strCardNo.Substring(1))+1;
                strRet = GetCardNo(iCount);
            }
            return strRet;
        }
        private static string GetCardNo(int iCount)
        {
            string strRet = "";
            string strCardNo = iCount.ToString();
            if (strCardNo.Length == 1)
            ...
            else if (strCardNo.Length == 6)
            {
                ...
                strRet = "B0000" + (iCount - 99999).ToString();
            }
            else
            {
                strRet = "C00000";
		...
		{
            BindProduct(gridProduct, null,null);

		}

        public static void BindProduct(UltraGrid gridProduct, string strProductName,string strMemberRight)
        {
            DataTable dtProductSort = GetProduct(strProductName, strMemberRight);
            gridProduct.SetDataBinding(dtProductSort, null, true, true);
