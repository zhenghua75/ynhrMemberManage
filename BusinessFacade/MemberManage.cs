	...
	public class MemberManageFacade
	{
		...
		public void RenewCard (Member Oldmember,PrintedBill pBill)
		{
			try
			{
				...
				//trans.Commit();
				CardM1 m1 = new CardM1();
				string strReturn = m1.PutOutCard("bbb"+member.cnvcMemberCardNo);
				if (strReturn.Equals("OPSUCCESS"))
				{
					trans.Commit();
				}
...
//				EntityMapping.Create(prepayLog,trans);
				...
				//trans.Commit();
				CardM1 m1 = new CardM1();
				string strReturn = m1.PutOutCard("bbb"+newMember.cnvcMemberCardNo);
				if (strReturn.Equals("OPSUCCESS"))
				{
					trans.Commit();
				}
		...
		public void ConsumeProduct(ArrayList alProduct,bool bMember,string strProduct,PrintedBill pBill)
		{
			try
			{
				...
				if (bMember)
				{
                    ...
                    if (oldMember.cnnPrepay < pBill.cnnPrepay) throw new BusinessException("服务产品消费", "预存金额不足，请充值！");
                    ...
                    //积分 20110211
                    oldMember.cnnPrepay = oldMember.cnnPrepay - pBill.cnnPrepay;
                    EntityMapping.Update(oldMember, trans);
                    PointsAdd(trans, ConstApp.Fee_Flag_Product, oldMember.cnvcMemberRight, productLog.cnvcMemberCardNo, productLog.cnvcOperName, productLog.cndOperDate, pBill.cnnPrepay);
                    /////////////////////////////////		
				...
				else
				{
                    ...
                    FMember oldfmember = new FMember();
                    oldfmember.cnvcPaperNo = fproductLog.cnvcPaperNo;
                    oldfmember = EntityMapping.Get(oldfmember,trans) as FMember;
                    if (oldfmember == null) throw new BusinessException("服务产品消费", "请录入非会员信息");
                    if (oldfmember.cnnPrepay < pBill.cnnPrepay) throw new BusinessException("服务产品消费", "预存金额不足，请充值！");
					for (int i = 0 ; i<alProduct.Count; i ++)
					{
                    ...
                    oldfmember.cnnPrepay = oldfmember.cnnPrepay - pBill.cnnPrepay;
...
//		}
        ...
        //    strsql = "select MAX(cnvcvalue) from tbNameCode where cnvcType like '充值赠送%-起始金额' and cnvcValue<='"+invalue.ToString()+"'";
        //    string strvalue = SqlHelper.ExecuteScalar(trans,CommandType.Text, strsql).ToString();
        //    if(string.IsNullOrEmpty(strvalue)) return 0;
        //    strsql = "select * from tbNameCode  where cnvcType like '充值赠送%-起始金额' and cnvcValue='"+strvalue+"'";
