...
namespace MemberManage.Business
{
	...
	public class JobManage
	{
		...
		public Job GetJobByID (Job job)
		{
			...
			{
				conn = ConnectionPool.BorrowConnection();
				//dtJob = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select cnnID as 招聘会ID,cnvcJobName as  招聘会名称,cnvcJobTheme as  招聘会主题,cndBeginDate as 招聘会开始时间, cndEndDate as 招聘会结束时间, cndBookBeginDate as 预订开始时间,cndBookEndDate as 预订结束时间 from tbJob where cnvcJobName like '%"+job.cnvcJobName+"%'");
				oldJob = EntityMapping.Get(job,conn) as Job;
		...
		public DataTable GetAllJob ()
		{
			...
			{
				conn = ConnectionPool.BorrowConnection();
				dtJob = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select * from tbJob");
		...
		public DataTable GetAll2DefaultSeat (string strID)
		{
			...
			{
				conn = ConnectionPool.BorrowConnection();
				dtSeat = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select * from tbShowSeat where cnnID="+strID+" and cnvcFloor='2'");
				if (dtSeat.Rows.Count == 0)
				{
					dtSeat = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select * from tbDefaultShowSeat where cnvcFloor='2'");
				}
		...
		public DataTable GetAll3DefaultSeat (string strID)
		{
			...
			{
				conn = ConnectionPool.BorrowConnection();
				dtSeat = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select * from tbShowSeat where cnnID="+strID+" and cnvcFloor='3'");
				if (dtSeat.Rows.Count == 0)
				{
					dtSeat = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select * from tbDefaultShowSeat where cnvcFloor='3'");
				}
		...
		public DataTable GetAllSeat (Job job)
		{
			...
			{
				conn = ConnectionPool.BorrowConnection();
				dtSeat = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select cnvcSeat as 展位,cnvcState as 状态 from tbShowSeat where cnnID = "+job.cnnJobID+" and cnvcSeat is not null order by cnvcSeat");
		...
		public DataTable GetFloorSeat (ShowSeat seat)
		{
			...
			{
				conn = ConnectionPool.BorrowConnection();
				dtSeat = SqlHelper.ExecuteDataTable(conn,CommandType.Text,"select * from tbShowSeat where cnnID = "+seat.cnnJobID+" and cnvcFloor = '"+seat.cnvcFloor+"'");
