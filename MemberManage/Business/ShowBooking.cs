...
namespace MemberManage.Business
{
    ...
    public class ShowBooking : frmBase
	{
		...
		private Infragistics.Win.UltraWinEditors.UltraTextEditor txtCFree;
		...
		private void InitializeComponent()
		{
            ...
            this.txtCFree = new Infragistics.Win.UltraWinEditors.UltraTextEditor();
            ...
            ((System.ComponentModel.ISupportInitialize)(this.txtCFree)).BeginInit();
            ...
            // ultraGroupBox4
            // 
            this.ultraGroupBox4.Controls.Add(this.txtCFree);
            ...
            // txtCFree
            // 
            this.txtCFree.DisplayStyle = Infragistics.Win.EmbeddableElementDisplayStyle.VisualStudio2005;
            this.txtCFree.Enabled = false;
            this.txtCFree.Location = new System.Drawing.Point(126, 136);
            this.txtCFree.Name = "txtCFree";
            this.txtCFree.Size = new System.Drawing.Size(100, 21);
            this.txtCFree.TabIndex = 15;
            // 
            // ultraLabel13
            // 
            this.ultraLabel13.Location = new System.Drawing.Point(14, 136);
            this.ultraLabel13.Name = "ultraLabel13";
            this.ultraLabel13.Size = new System.Drawing.Size(100, 23);
            this.ultraLabel13.TabIndex = 14;
            this.ultraLabel13.Text = "场次：";
            // 
            // btnCancelCardBook
            // 
            ...
            this.btnCancelCardBook.Location = new System.Drawing.Point(168, 168);
            ...
            this.btnBrushCard.Location = new System.Drawing.Point(8, 168);
            ...
            this.btnCardBook.Location = new System.Drawing.Point(88, 168);
            ...
            this.Text = Login.constApp.strCardTypeL8Name + "展位预订";
            ...
            ((System.ComponentModel.ISupportInitialize)(this.txtCFree)).EndInit();
...
//						   +" where a.cnvcFloor='"+strFloor+"' and a.cnnJobID="+strJobID;
            ...
            //info.SetValue(pl, true, null);
            //List<zhhLabel> lctrl = new List<zhhLabel>();
			foreach (DataRow drSeat in dtSeat.Rows)
			{
                ...
                ((System.ComponentModel.ISupportInitialize)(lbl)).BeginInit();
...
//					}
                ...
                pl.Controls.Add(lbl);
                ((System.ComponentModel.ISupportInitialize)(lbl)).EndInit();
                //lctrl.Add(lbl);
					
			}
           
            //pl.Controls.AddRange(lctrl.ToArray());
            //pl.ResumeLayout();
		...
		private void btnQuery_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string strSql = "select top 20 cnvcMemberCardNo,cnvcMemberName,cnvcPaperNo,cnvcMemberRight,cnvcFree,'' as cnvcBookSeat ,'' as cnvcJobName from tbMember where cnvcState = '"+ConstApp.Card_State_InUse+"'";
		...
		private void btnBooking_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                if (selectedRow.Cells["cnvcMemberCardNo"].Value.ToString().Length == 6) throw new BusinessException("会员预定", "请选择会员卡（8位）");
...
//				DataTable dtState1 = Helper.Query("select * from tbMember where  cast(convert(varchar(10),cndEndDate,120) as DateTime) < '"+ strTime +"' and cnvcMemberCardNo='" +seat.cnvcMemberCardNo+"'");
...
//				}
				...
				if (dtState1.Rows.Count > 0)
				{
					Member tMember = new Member(dtState1);
					...
					{
						if (double.Parse(tMember.cnvcFree)<=0)
						{
				...
				if (ret == DialogResult.Yes)
				{
					...
					if (IsSignIn)
					{
						...
						if (dr2 == DialogResult.Yes)
						{
							...
							string strSeat = jobManage.FMemberSeatSignIn(memberSeat);
...
//		}
		...
		private void btnBrushCard_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
                ...
                if (strCardNo.Length == 6) throw new BusinessException("会员刷卡预定", "请放入会员卡（8位）");
                ...
                {
                    throw new BusinessException("展位预定", "请核对该会员是否已经发6位“一通卡”，未发请用卡号预定和签到！！");
                }
				string strTime="";
				string strTimeY="";
				string strTimeM="";
		...
		private void btnCardBook_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (ret == DialogResult.Yes)
				{
					...
					{
						Member tMember = new Member(dtState1);
						DateTime dtMemberEndDate = DateTime.Parse(tMember.cndEndDate);						
						if (dtMemberEndDate < job.cndBeginDate)
						{
							DialogResult drBook = MessageBox.Show(this,"招聘会开始时会员已过期，是否继续预定？","展位预订",MessageBoxButtons.YesNo,MessageBoxIcon.Information);
							if (drBook == DialogResult.No)
							...
							if (tMember.cnvcFree != ConstApp.Free_Time_NoLimit)
							{
								if (double.Parse(tMember.cnvcFree)<=0)
								{
									drBook = MessageBox.Show(this,"免费场次为0，是否继续预定？","展位预订",MessageBoxButtons.YesNo,MessageBoxIcon.Information);
									if (drBook == DialogResult.No)
									{
										return;
									}
					...
					if (IsSignIn)
					{
                        ...
                        DataTable dtMemberIn1 = Helper.Query("select * from tbMember where cnvcMemberCardNo = '" + seat.cnvcMemberCardNo + "' and cndInNetDate < '20110228'");
                        if (dtMemberIn1.Rows.Count != 0)
                        {
                            throw new BusinessException("展位签到", "请核对该会员是否已经发6位“一通卡”，未发请用卡号和密码签到！！");
                        }
						DialogResult dr2= MessageBox.Show(this,"是否立即签到？","签到",MessageBoxButtons.YesNo,MessageBoxIcon.Information);
						if (dr2 == DialogResult.Yes)
						{
