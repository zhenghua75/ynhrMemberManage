...
namespace MemberManage.Business.ConsumeBusiness
{
    ...
    public class ShowSignIn : frmBase
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Text = Login.constApp.strCardTypeL6Name + "展位签到";
...
//					{
		...
		private void btnSignIn_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
                ...
                if (!this.GetIsJob(oldMember.cnvcMemberRight))
                {
                    throw new BusinessException("展位签到", "此会员不能参加招聘会");
                }
                bool bFeeType = this.GetIsFeeType(oldMember.cnvcMemberRight);
				Job job = new Job();
				job.cnnJobID = ms.cnnID;
				job.cnvcJobName = ms.cnvcJobName;
                if (this.GetIsDisabledDate(oldMember.cnvcMemberRight))
                {
                ...
                pBill.cnvcSynch = bill.cnvcSynch;
				btnSignIn.Enabled = false;
		...
		private void MemberBook(Member member,Job job,string strFloor,string strSeat)
		{
            ...
            if (!this.GetIsJob(strMemberRight))
            {
                throw new BusinessException("展位预订", "此会员不能参加招聘会");
            }
		...
		private void MemberSignIn_Click(object sender, System.EventArgs e)
		{
			...
			{			
                ...
                bool bFeeType = this.GetIsFeeType(member.cnvcMemberRight);
		...
		private void FMemberSignIn_Click(object sender, System.EventArgs e)
		{
			...
			{			
				...
				if (ret == DialogResult.Yes)
				{
					...
					JobManage jobManage = new JobManage();
                    Bill bill = null;
					string strSeats = jobManage.FMemberSeatSignIn(memberSeat,out bill);
		...
		private void BrushCard_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
                ...
                if (strCardNo.Length > 6) throw new BusinessException("刷卡签到", "请放入一卡通");
		...
		private void btnMemberQuery_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
                ...
                string strSql = "select top 10 cnvcMemberCardNo,cnvcMemberName,cnvcPaperNo,cnvcMemberRight,cnnPrepay,'' as cnvcBookSeat,cnvcMemberPwd from tbMember  where LEN(cnvcMemberCardNo)=6 and cnvcState = '" + ConstApp.Card_State_InUse + "'";
		...
		private void ultraGrid1_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
		{
            ...
            e.Layout.Bands[0].Columns["cnnPrepay"].Width = 60;
		}
		...
		private void btnFMemberQuery_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string strSql = "select top 10 cnvcMemberName,cnvcPaperNo,cnnPrepay,'' as cnvcBookSeat from tbFMember where 1=1";				
                ...
                dv.Sort = "cnvcBookSeat desc";
				this.ultraGrid2.DataSource = null;
				this.ultraGrid2.DataSource = dv;
